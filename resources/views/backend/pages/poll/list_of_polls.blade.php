...
<script>
    $(document).ready(function(){
        $(document).on('click', '.display-toggle', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            var newDisplay = $(this).data('display');
            var url = "{{ route('poll.display', ':id') }}".replace(':id', id);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.display === 'show') {
                        $('.display-toggle[data-id="' + id + '"]').html('<span class="badge bg-label-success">Show</span>');
                        ...
                        $('.display-toggle[data-id="' + id + '"]').html('<span class="badge bg-label-danger">Hide</span>');
                        // toastr.success(response.notify.message);
                        Swal.fire({
                            title: 'Good job!',
                            text:response.notify.message,
            ...
            $(document).on('click', '.delete-btn', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = "{{ route('delete.poll', ':id') }}".replace(':id', id);
            ...
            $(document).on('click', '.ban-btn', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = "{{ route('ban.poll', ':id') }}".replace(':id', id);
...
<script>
    $(function(){
        ...
        var addButton = "Add New Poll";
        var tableTitle = "List Of Polls";
        var columnNumber = [1,2,3,4,5,6];
        var url = "{{ route('list.poll') }}";
        let table =$('#table');
        ...
        $(document).on('click', '.delete-all', function(e){
            ...
            Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = "{{ route('poll.delete.multiple') }}";
                        $.ajax({
                            url: url,
                            type: 'POST',
                            data: {
        ...
        $(document).on('click', '.ban-all', function(e){
            ...
            Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var url = "{{ route('poll.ban.multiple') }}";
                        $.ajax({
                            url: url,
                            type: 'POST',
                            data: {
