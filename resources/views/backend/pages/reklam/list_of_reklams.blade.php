    ...
    <div class="card">
        ...
        <div class="card">
            <div class="card-datatable pt-0">
                ...
                <a href="{{ route('add.reklam') }}" class="btn btn-secondary add-new btn-primary waves-effect waves-light m-2"><span><i
                ...
                <table class="table" id="zakho_table">
                    ...
                    <tbody>
                        @foreach ($reklams as $index => $reklam)
                        <tr>
                            <td><input class="form-check-input" type="checkbox" id="select"></td>
                            <td>{{ $index+1 }}</td>
                            <td>
                                <input type="hidden" value="{{ $reklam->id }}">
                                <div class="d-flex justify-content-start align-items-center user-name">
                                    <div class="avatar-wrapper">
                                        <div class="avatar me-3"><img src="{{ !empty($reklam->image)? asset($reklam->image) : 'https://via.placeholder.com/150x150' }}" alt="Avatar"
                                                class="rounded-circle"></div>
                                    </div>
                                    <div class="d-flex flex-column"><a href="{{ $reklam->url}}"
                                            class="text-body text-truncate"><span
                                                class="fw-medium">{{ Illuminate\Support\Str::of($reklam->title)->apa() }}</span></a></div>
                                </div>
                            </td>
                            <td>
                                @if ($reklam->status == 'active')
                                <a href="javascript:void(0);" class="status-toggle" data-id="{{ $reklam->id }}" data-status="inactive"><span class="badge bg-label-success">Active</span></a>
                            @else
                                <a href="javascript:void(0);" class="status-toggle" data-id="{{ $reklam->id }}" data-status="active"><span class="badge bg-label-danger">Inactive</span></a>
                            @endif
                            </td>
                            <td>
                                {{ $reklam->user->name }}
                            ...
                            <td>
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li><a href="{{ route('edit.reklam',$reklam->id) }}" class="dropdown-item"><i
                                                class="ti ti-edit"></i> Edit</a></li>
                                    <li><a href="javascript:void(0);" class="dropdown-item " id="ban-btn" data-id="{{ $reklam->id }}"><i class="ti ti-ban"></i> Ban</a></li>
                                    <li><a href="javascript:void(0);" class="dropdown-item " id="delete-btn" data-id="{{ $reklam->id }}"><i class="ti ti-trash"></i> Delete</a></li>
                                </ul>
                            </td>
                        </tr>
                        @endforeach
